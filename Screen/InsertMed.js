...
export default InsertMed = ({navigation,props,route}) =>  {
    ...
    const add = async (props) => {
        ...
        const requestOptions = {
            ...
            body: JSON.stringify({ Name:Name, Qty:Qty,Price:Price,Category:Category,Store:c  })
        };
        try {
            await fetch(
                ipCon.ip+"/addMedToStock", requestOptions)
                .then(res => {
                    res.json()
                        .then(data => {
    ...
    return (
        ...
        <TouchableOpacity onPress={() => navigation.navigate('MerchantDashboard')}>
            ...
            <Text style={styles.Meditext}>Add to store</Text>
            </View>
